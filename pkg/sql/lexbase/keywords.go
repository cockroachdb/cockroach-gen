...
func GetKeywordID(k string) int32 {
	...
	case "incremental":
		return INCREMENTAL
	case "incremental_location":
		return INCREMENTAL_LOCATION
	case "index":
		return INDEX
	case "indexes":
		return INDEXES
	case "index_after_order_by_before_at":
		return INDEX_AFTER_ORDER_BY_BEFORE_AT
	case "index_before_name_then_paren":
		return INDEX_BEFORE_NAME_THEN_PAREN
	case "index_before_paren":
		return INDEX_BEFORE_PAREN
	case "inherits":
		return INHERITS
	case "initially":
		return INITIALLY
	...
	case "nothing_after_returning":
		return NOTHING_AFTER_RETURNING
	case "notnull":
		return NOTNULL
	case "noviewactivity":
